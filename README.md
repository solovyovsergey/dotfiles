...
После чего применить новый конфиг командой `source ~/.zsh`
...
Обычно ставится просто через пакетный менеджер. Подробнее здесь <https://github.com/tmux/tmux/wiki/Installing>

### Подключаем файл конфигурации .tmux.conf

Файл конфигурации хранится здесь `~/.tmux.conf`
...
# Создать символьную ссылку на файл из dotfiles
ln -s ~/path/to/your/dotfiles/tmux/.tmux.conf ~/.tmux.conf

```

После чего применить новый конфиг командой `tmux source-file ~/.tmux.conf`

### ⌨️ Tmux Hotkeys Cheat Sheet

**Префикс по умолчанию:** `Ctrl + a`

#### 🖥️ Управление панелями (Panes)

- **`Prefix` + `\`** или **`|`** — Разделить экран **вертикально**
- **`Prefix` + `-`** — Разделить экран **горизонтально**
- **`Prefix` + `h/j/k/l`** — Переключиться на панель (влево/вниз/вверх/вправо)
- **`Prefix` + `Shift` + `H/J/K/L`** — Изменить **размер** активной панели
- **`Ctrl + d`** или `exit` — Закрыть текущую панель

#### 📑 Управление окнами (Windows)

- **`Prefix` + `c`** — Создать новое окно (вкладку)
- **`Prefix` + `1...9`** — Перейти к окну по номеру
- **`Prefix` + `&`** — Закрыть текущее окно целиком

#### 📝 Режим копирования и история (Copy Mode)

- **`Prefix` + `[`** — Вход в режим просмотра истории
- **`h/j/k/l`** — Навигация по тексту (как в Vim)
- **`/`** — Поиск по истории вывода
- **`v`** — Начать выделение текста
- **`y`** — Скопировать и выйти (текст попадет в системный буфер)
- **`q`** — Просто выйти из режима просмотра

#### ⚙️ Сервисные команды

- **`Prefix` + `r`** — Мгновенная **перезагрузка конфигурации**
- **`Prefix` + `s`** — Список всех активных сессий
- **`Prefix` + `d`** — Отключиться от tmux (сессия продолжит работать в фоне)
...
Перечисленные утилиты уже поддерживаются конфигом `zsh`. Потому, после их установки, нужно проверить `~/.zsh`, чтобы удалить дубли настроек, которые скрипты установщики дописывают в конец файла.

1. `zoxide` - умный аналог cd.
2. `fzf` - инструмент для поиска нечетких данных.
   - Ctrl + t — Поиск файла в текущей директории и подпапках
   - Alt + c — Быстрый переход (cd) в подпапку.
   - Ctrl + r — Поиск по истории команд.
3. `bat` - аналог `cat` с поддержкой подсветки синтаксиса для большинства языков.
4. `eza` - маленький аналог ls с поддержкой цветов.
